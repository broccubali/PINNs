...
def bc(u, dx, Ncell, mode="periodic", noise_level=0.0, scale = 1.25):
    _u = jnp.zeros(Ncell + 4)  # because of 2nd-order precision in space
    _u = _u.at[2 : Ncell + 2].set(u)
    noise = np.random.exponential(scale=scale, size=_u.shape) * noise_level    
...
def main() -> None:
    ...
    @jax.jit
    def update(u, u_tmp, dt, scale = 1.25):
        f = flux(u_tmp)
        noise = np.random.exponential(scale=scale, size=f.shape) * equation_noise_level
    ...
    exponential_noise = np.random.exponential(scale=1.25, size=u.shape) * noise_level
    u = u + exponential_noise
    ...
    print("data saving...")
    jnp.save(
        "pde-gen/burgers/data/burgerNoisy.npy",
        uu
    )
