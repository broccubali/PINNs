...
def bc(u, dx, Ncell, mode="periodic", noise_level=0.0):
    _u = jnp.zeros(Ncell + 4)  # because of 2nd-order precision in space
    _u = _u.at[2 : Ncell + 2].set(u)
    noise = np.random.standard_cauchy(_u.shape) * noise_level
...
def main() -> None:
    ...
    def update(u, u_tmp, dt):
        f = flux(u_tmp)
        noise = np.random.standard_cauchy(f.shape) * equation_noise_level
    ...
    cauchy_noise = np.random.standard_cauchy(u.shape) * noise_level
    u = u + cauchy_noise
