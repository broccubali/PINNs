...
def generate_parabolic_data(
    grid_size=(32, 32), num_samples=100, time_steps=50, noise_level=0.1
):
    data_info = []
    data_samples = []
...
def generate_hyperbolic_data(
    ...
    for i in range(num_samples):
        ...
        u_prev[grid_size[0] // 2, grid_size[1] // 2] = (
            1  # Initial disturbance at center
        )
...
class PINN_Elliptic(tf.keras.Model):
    def __init__(self):
        ...
        self.hidden_layers = [
            tf.keras.layers.Dense(100, activation="tanh") for _ in range(8)
        ]
...
parabolic_data, parabolic_summary = generate_parabolic_data(
    noise_level=0.3, num_samples=5, time_steps=50
)
hyperbolic_data, hyperbolic_summary = generate_hyperbolic_data(
    noise_level=0.3, num_samples=5, time_steps=50
)
...
plt.subplot(131)
plt.contourf(
    elliptic_data[0][0], elliptic_data[0][1], elliptic_data[0][2], cmap="viridis"
)
...
plt.subplot(132)
plt.contourf(
    parabolic_data[0][0], parabolic_data[0][1], parabolic_data[0][2], cmap="viridis"
)
...
plt.subplot(133)
plt.contourf(
    hyperbolic_data[0][0], hyperbolic_data[0][1], hyperbolic_data[0][2], cmap="viridis"
)
...
for epoch in epochs_range:
    ...
    loss_value_parabolic = parabolic_pde_loss(
        parabolic_model, x_tensor_parabolic, y_tensor_parabolic, t_tensor_parabolic
    )
    ...
    loss_value_hyperbolic = hyperbolic_pde_loss(
        hyperbolic_model, x_tensor_hyperbolic, y_tensor_hyperbolic, t_tensor_hyperbolic
    )
